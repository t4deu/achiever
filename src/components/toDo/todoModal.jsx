...
import ListToDoItem from '../listToDoItem.jsx';
...
class ToDoModal extends Component {
  ...
  toDoList(toDosArr) {
    const htmlList = toDosArr.map((todo) => (
      <ListToDoItem
    ...
    return <ul className="todos-list">{htmlList}</ul>;
  }
      ...
      <div className="todo-modal">
        <div className="status-and-list">
          <div className="todoList-status">
            {this.props.amountUndoneToDos} to do
          </div>
          {this.props.toDosArr.length !== 0 && (
            <div className="todoList-container">
              {this.toDoList(this.props.toDosArr)}
            </div>
          )}
          {this.props.toDosArr.length === 0 && (
            <div className="no-todo-container">
              <div>
                <img className="smile" src="/assets/images/smile.svg" />
              </div>
        ...
        <div className="close" onClick={this.props.toggleModal.bind(this)}>
          &times;
        </div>
      </div>
    );
