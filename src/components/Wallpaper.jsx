...
export default class WallpaperInfo extends React.Component {
  constructor(props) {
    super(props);
    const haveTodaysPhoto = isNotANewDay(
      localStorage.wallpaperTimestamp,
      getCurrentTime()
    );
    if (localStorage.wallpaperTimestamp === '0' || haveTodaysPhoto) {
      ...
      this.preLoadImages(
        this.props.wallpaperData.urls.regular,
        this.props.wallpaperData.urls.thumb
      );
    } else {
      this.state = {
        divStyle: {},
  ...
  componentWillReceiveProps(nextProps) {
    if (
      nextProps.wallpaperData &&
      this.state.wallpaperData.id !== nextProps.wallpaperData.id
    ) {
      this.setState({
        wallpaperData: nextProps.wallpaperData,
      });
      this.preLoadImages(
        nextProps.wallpaperData.urls.regular,
        nextProps.wallpaperData.urls.thumb
      );
    }
  }
    ...
    if (!this.state.haveTodaysPhoto) {
      getUnsplashPhoto()
        .then((result) => {
          ...
          wallpaperData.wallpaperLiked =
            localStorageKeyExists('wallpaper') &&
            objIsInArray(
              getFromLocalStorage('arrLikedWallpapers'),
              'id',
              wallpaperData.id
            );
          this.preLoadImages(
            wallpaperData.urls.regular,
            wallpaperData.urls.thumb
          );
          this.setState({
            wallpaperData,
          });
        ...
        .catch((error) => {
          ...
          this.setState({
            divStyle: {
              backgroundImage:
                'url(/assets/images/photo-1476616853026-24c1f0309646.jpg)',
            },
          });
        });
    }
    ...
    if (this.state.showThumb) {
      return (
        <div
          ...
          style={this.state.thumbdDivStyle}
        ></div>
      );
    }
    return (
      <div className="wallpaper-container" style={this.state.divStyle}></div>
    );
  }
}
